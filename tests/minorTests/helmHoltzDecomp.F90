...
#include "petsc/finclude/petscsys.h"
    ...
    subroutine decomposeHelmholtz_2(uvel, vvel, psi, phi, centerDx, centerDy, dxBottomE, dxTopE, dyLeftE, dyRightE, &
        ...
        call solvepoissionBig(psi, phi, uvel, vvel, centerDx, centerDy, dxBottomE, dxTopE, dyLeftE, dyRightE, &
                                &dxLeftN, dxRightN, dyBottomN, dyTopN,cellArea)
    ...
    subroutine solvepoissionBig(psi, phi, uvel, vvel, centerDx, centerDy, dxBottomE, dxTopE, dyLeftE, dyRightE, &
        dxLeftN, dxRightN, dyBottomN, dyTopN, cellArea)
        ...
        PetscScalar :: leftCoeff, rightCoeff, topCoeff, bottomCoeff, centerCoeff, &
                       lleftCoeff, rrightCoeff, ttopCoeff, bbottomCoeff, &
                       llleftCoeff, rrrightCoeff, tttopCoeff, bbbottomCoeff 
        ...
        real(kind=real_kind), allocatable :: collected_xarray(:), divU(:,:), curlU(:,:)!, &
                                             ...
                                             !uvel_tor(:,:), uvel_pol(:,:), vvel_tor(:,:), vvel_pol(:,:) 
        logical :: inColRange
        ...
        if (rank == 0) then
            ! allocate(uvel_res(mx, my), vvel_res(mx, my), phi_seed(mx, my), psi_seed(mx, my), &
            !          uvel_tor(mx, my), uvel_pol(mx, my), vvel_tor(mx, my), vvel_pol(mx, my), stat=ierr)

            ! call getPolTorVel(psi, phi, centerDx, centerDy, dxBottomE, dxTopE, dyLeftE, dyRightE, cellArea, &
            ...
            ! deallocate(uvel_tor, uvel_pol, vvel_tor, vvel_pol)
        endif
        ...
        if (rank == 0) then
            ...
            do Ii = 0, mx * my - 1
                ...
                if (i == mx/2 .and. j == my/2 ) print *, 'psi, phi', psi(i+1, j+1), phi(i+1, j+1)

                !!!!!!!!!!!!!!!!!!!!!!!!!! setting RHS !!!!!!!!!!!!!!!!!!!!!!!
                val = uvel(i+1, j+1) * cellArea(i+1, j+1) * derFac   
                !val = uvel_res(i+1, j+1) * cellArea(i+1, j+1) * derFac   
                call VecSetValue(y_globalOnZero, Ii, val, INSERT_VALUES, ierr)
                
                val = vvel(i+1, j+1) * cellArea(i+1, j+1) * derFac
                !val = vvel_res(i+1, j+1) * cellArea(i+1, j+1) * derFac
        ...
        do Ii = Istart, Iend - 1
            ...
            if (Ii < 2 * mx * my) then
                if (i == 0) then
                    ...
                    rrrightCoeff = 0
                    rrightCoeff = -1./(12 * dxRightN(i+1, j+1))
                    rightCoeff = 2./(3 * dxRightN(i+1, j+1))
                    centerXCoeff = 0
                    leftCoeff = -2./(3 * dxRightN(i+1, j+1))
                    lleftCoeff = 1./(12 * dxRightN(i+1, j+1))
                    llleftCoeff = 0

                else
                    rrrightCoeff = 1./(60 * dxRightN(i+1, j+1))
                    rrightCoeff = -3./(20 * dxRightN(i+1, j+1))
                    rightCoeff = 3./(4 * dxRightN(i+1, j+1))
                    centerXCoeff = 0
                    leftCoeff = -3./(4 * dxRightN(i+1, j+1))
                    lleftCoeff = 3./(20 * dxRightN(i+1, j+1))
                    llleftCoeff = -1./(60 * dxRightN(i+1, j+1))
                endif
                ...
                if (j == 0 ) then
                    ...
                    tttopCoeff = 0
                    ttopCoeff = -1./(12 * dyTopN(i+1, j+1))
                    topCoeff = 2./(3* dyTopN(i+1, j+1))
                    centerYCoeff = 0
                    bottomCoeff = -2./(3*dyTopN(i+1, j+1)) 
                    bbottomCoeff = 1./(12 * dyTopN(i+1, j+1)) 
                    bbbottomCoeff = 0         
                else
                    tttopCoeff = 1./(60 * dyTopN(i+1, j+1))
                    ttopCoeff = -3./(20 * dyTopN(i+1, j+1))
                    topCoeff = 3./(4* dyTopN(i+1, j+1))
                    centerYCoeff = 0
                    bottomCoeff = -3./(4*dyTopN(i+1, j+1)) 
                    bbottomCoeff = 3./(20 * dyTopN(i+1, j+1)) 
                    bbbottomCoeff = -1./(60 * dyTopN(i+1, j+1))     
                endif
                ...
                if (Ii < mx *my) then
                    ...
                    if (i > 1 )  then
                        colIndex = Ii - 2
                        if ( colIndex >= 2 *mx *my ) stop 'Here 01'
                        call MatSetValue(A, Ii, colIndex , -lleftCoeff *derFac  * cellArea(i+1,j+1) , ADD_VALUES, ierr)
                    endif

                    if (i > 2 )  then
                        colIndex = Ii - 3
                        if ( colIndex >= 2 *mx *my ) stop 'Here 01__'
                        call MatSetValue(A, Ii, colIndex , -llleftCoeff *derFac  * cellArea(i+1,j+1) , ADD_VALUES, ierr)
                    endif
                    ...
                    if (i < mx -2 )  then
                        colIndex = Ii + 2
                        if ( colIndex >= 2 * mx *my ) stop 'Here 02'
                        call MatSetValue(A, Ii, colIndex , -rrightCoeff *derFac * cellArea(i+1,j+1), ADD_VALUES, ierr)
                    endif

                    if (i < mx -3 )  then
                        colIndex = Ii + 3
                        if ( colIndex >= 2 * mx *my ) stop 'Here 02'
                        call MatSetValue(A, Ii, colIndex , -rrrightCoeff *derFac * cellArea(i+1,j+1), ADD_VALUES, ierr)
                    endif

                    colIndex = Ii
                    if ( colIndex >= 2 *mx *my ) stop 'Here 03'
                    call MatSetValue(A, Ii, colIndex, -centerXCoeff *derFac * cellArea(i+1,j+1) -tikhov, ADD_VALUES, ierr)
                    ...
                    if (j > 1 )   then
                        ...
                        if ( colIndex >= 2*mx *my ) stop 'Here 04'
                        call MatSetValue(A, Ii, colIndex, bbottomCoeff *derFac * cellArea(i+1,j+1), ADD_VALUES, ierr) 
                    endif

                    if (j > 2 )   then
                        colIndex = (Ii + mx * my) - mx - mx -mx
                        if ( colIndex >= 2*mx *my ) stop 'Here 04'
                        call MatSetValue(A, Ii, colIndex, bbbottomCoeff *derFac * cellArea(i+1,j+1), ADD_VALUES, ierr) 
                    endif
                    ...
                    if (j< my -2 )   then
                        colIndex = (Ii + mx * my) + mx + mx
                        if ( colIndex >= 2*mx *my ) stop 'Here 05'
                        call MatSetValue(A, Ii, colIndex, ttopCoeff *derFac * cellArea(i+1,j+1) , ADD_VALUES, ierr)
                    endif

                    if (j< my -3 )   then
                        colIndex = (Ii + mx * my) + mx + mx + mx
                        if ( colIndex >= 2*mx *my ) stop 'Here 05'
                        call MatSetValue(A, Ii, colIndex, tttopCoeff *derFac * cellArea(i+1,j+1) , ADD_VALUES, ierr)
                    endif

                    colIndex = (Ii + mx * my)
                    if ( colIndex >= 2*mx *my ) stop 'Here 06'
                    call MatSetValue(A, Ii, colIndex, centerYCoeff *derFac * cellArea(i+1,j+1)-tikhov, ADD_VALUES, ierr)
                ...
                if ((i > 0) .and. (i < mx - 1) .and. (j > 0) .and. (j < my - 1)) then
                    ...
                    bottomCoeff = dxBottomE(i+1,j+1)/dyBottomN(i+1,j+1)
                    centerCoeff = -dyLeftE(i+1,j+1)/dxLeftN(i+1,j+1) - dyRightE(i+1,j+1)/dxRightN(i+1,j+1) &
                                & -dxBottomE(i+1,j+1)/dyBottomN(i+1,j+1) - dxTopE(i+1,j+1)/dyBottomN(i+1,j+1) !- 0.1
                    ...
                    call MatSetValue(A, Ii,  colIndex2, centerCoeff -tikhov, ADD_VALUES, ierr)
    ...
    subroutine solvepoissionBigFD(psi, phi, uvel, vvel, dx, dy)
        ...
        PetscInt :: Istart, Iend, Jstart, Jend, Ii, Ji, i, j, mx, my, &
                    localSize, globalSize, low, high
        PetscInt :: indx_phi, indx_psi, indx_u, indx_v, indx_div, indx_curl, colIndex, colIndex2
        PetscScalar :: leftCoeff, lleftCoeff, rightCoeff, rrightCoeff, topCoeff, ttopCoeff,&
                       bottomCoeff, bbottomCoeff, centerCoeff
        PetscScalar :: leftGradCoeff, lleftGradCoeff, rightGradCoeff, rrightGradCoeff, topGradCoeff, &
                       ttopGradCoeff, bottomGradCoeff, bbottomGradCoeff,  &
                       centerGradXCoeff, centerGradYCoeff, centerXCoeff, centerYCoeff
        ...
        do Ii = Istart, Iend - 1
            ...
            if (Ii < 2 * mx * my) then
                if (i == 0) then
                    !!! Setting gradient coefficients
                    rrightCoeff = 0
                    rightCoeff = 1.0/dx(i+1, j+1)
                    centerXCoeff = -1.0/dx(i+1, j+1)
                    leftCoeff = 0
                    lleftCoeff = 0
                else if (i == mx-1) then
                    rrightCoeff = 0
                    ...
                    lleftCoeff =0
                else if (i==1 .or. i == mx-2) then
                    rrightCoeff = 0
                    rightCoeff = 0.5 * 1./dx(i+1, j+1)
                    centerXCoeff = 0
                    leftCoeff =  -0.5 * 1./dx(i+1, j+1)
                    lleftCoeff = 0
                else
                    rrightCoeff = -1./12.0 * 1./dx(i+1, j+1)
                    rightCoeff = 2./3. * 1./dx(i+1, j+1)
                    centerXCoeff = 0
                    leftCoeff =  -2./3. * 1./dx(i+1, j+1)
                    lleftCoeff =  1./12.0 * 1./dx(i+1, j+1)
                endif
                ...
                if (j == 0 ) then
                    ...
                    bbottomCoeff = 0
                else if (j ==1 .or. j==my-2) then
                    ttopCoeff =0 
                    topCoeff = 0.5 * 1./dy(i+1, j+1)
                    centerYCoeff = 0
                    bottomCoeff =  -0.5 * 1./dy(i+1, j+1)  
                    bbottomCoeff =0  
                else
                    ttopCoeff = -1./12.0 * 1./dy(i+1, j+1)
                    topCoeff = 2./3. * 1./dy(i+1, j+1)
                    centerYCoeff = 0
                    bottomCoeff =  -2./3. * 1./dy(i+1, j+1) 
                    bbottomCoeff =  1./12.0 * 1./dy(i+1, j+1) 
                endif
                ...
                if (Ii < mx *my) then
                    ...
                    if (i > 1 )  then
                        colIndex = Ii - 2
                        if ( colIndex >= 2 *mx *my ) stop 'Here 01_'
                    ...
                    if (i < mx -2 )  then
                        colIndex = Ii + 2
                        if ( colIndex >= 2 * mx *my ) stop 'Here 02_'
                    ...
                    if (j > 1 )   then
                        colIndex = (Ii + mx * my) - mx - mx
                        if ( colIndex >= 2*mx *my ) stop 'Here 04_'
                    ...
                    if (j< my -2 )   then
                        colIndex = (Ii + mx * my) + mx +  mx
                        if ( colIndex >= 2*mx *my ) stop 'Here 05_'
                    ...
                    if (j > 1 )    then
                        colIndex = Ii -(mx * my) - mx - mx 
                        if ( colIndex >= 2*mx *my ) stop 'Here 07_'
                        call MatSetValue(A, Ii, colIndex , -bbottomCoeff, ADD_VALUES, ierr)
                    endif
                    ...
                    if ( j< my -2 )  then
                        colIndex = Ii -(mx * my) + mx + mx
                        if ( colIndex >= 2*mx *my ) stop 'Here 08_'
                        call MatSetValue(A, Ii, colIndex , -ttopCoeff, ADD_VALUES, ierr)
                    endif
                    ...
                    if (i > 1 )     then
                        colIndex = Ii  - 2
                        if ( colIndex >= 2*mx *my ) stop 'Here 010_'
                        call MatSetValue(A, Ii, colIndex , -lleftCoeff, ADD_VALUES, ierr)
                    end if
                    ...
                    if (i < mx -2)  then
                        colIndex = Ii  + 2
                        if ( colIndex >= 2*mx *my ) stop 'Here 011_'
                        call MatSetValue(A, Ii, colIndex , -rrightCoeff, ADD_VALUES, ierr)
                    endif
                ...
                ! Interior grid points: central difference stencil
                if (i == 0)  then
                    rrightCoeff = 1./dx(i+1, j+1)
                    rightCoeff = -2./dx(i+1, j+1)
                    centerCoeff = 1./dx(i+1, j+1)
                    leftCoeff = 0.0
                    lleftCoeff = 0.0
                ...
                elseif (i == mx -1) then
                    ...
                    rightCoeff = 0.0
                    centerCoeff = 1./dx(i+1, j+1)
                    leftCoeff = -2./dx(i+1, j+1)
                    lleftCoeff = 1./dx(i+1, j+1)

                
                else
                    rrightCoeff = -1./12.0 * 1./dx(i+1, j+1)
                    rightCoeff = 4.0/3.0 * 1./dx(i+1, j+1)
                    centerCoeff = -5./2.0 * 1./dx(i+1, j+1)
                    leftCoeff = 4.0/3.0 * 1./dx(i+1, j+1)
                    lleftCoeff = -1./12.0 * 1./dx(i+1, j+1)
                ...
                if (j == 1) then
                    ttopCoeff = 1./dy(i+1, j+1)
                    topCoeff = -2./dy(i+1, j+1)
                    centerCoeff = centerCoeff + 1./dy(i+1, j+1)
                    bottomCoeff = 0
                    bbottomCoeff = 0
                ...
                elseif (j == 2 .or. j == my - 2) then
                    ttopCoeff = 0
                    topCoeff = 1./dy(i+1, j+1)
                    centerCoeff = centerCoeff -2./dy(i+1, j+1)
                    bottomCoeff = 1./dy(i+1, j+1)
                    bbottomCoeff = 0
                ...
                elseif (j == my - 1) then
                    ...
                    topCoeff = 0
                    centerCoeff = centerCoeff + 1./dy(i+1, j+1)
                    bottomCoeff =  -2./dy(i+1, j+1)
                    bbottomCoeff =  1./dy(i+1, j+1)

                
                else
                    !!! setting nine coefficients for each row based on Finite Volume Descretization in the center points
                    ttopCoeff = -1./12.0 * 1./dy(i+1, j+1)
                    topCoeff = 4.0/3.0 * 1./dy(i+1, j+1)
                    centerCoeff = centerCoeff + -5.0/2.0 * 1./dy(i+1, j+1)
                    bottomCoeff = 4.0/3.0 * 1./dy(i+1, j+1)
                    bbottomCoeff = -1./12.0 * 1./dy(i+1, j+1)
    ...
    subroutine solvepoissionBig_LHSRHS(LHS, RHS, dxBottomE, dxTopE, dyLeftE, dyRightE, &
        ...
        if (rank == 0) print *, 'vectors for LHS and RHS created'
        ...
        if (rank == 0) then
            ...
            call VecGetArrayReadF90(sol_globalOnZero, collected_xPointer, ierr)
            collected_xarray = collected_xPointer
            LHS = collected_xarray
            deallocate(collected_xarray)
            nullify(collected_xPointer)

            allocate(collected_xarray(4*mx*my))
            call VecGetArrayReadF90(y_globalOnZero, collected_xPointer, ierr)
            collected_xarray = collected_xPointer
            RHS = collected_xarray
            deallocate(collected_xarray)
            nullify(collected_xPointer)
